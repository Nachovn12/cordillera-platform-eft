# Prompt de Implementación - CORD-119
...
- **HU padre:** CORD-119 — Configurar quality gate JaCoCo 60% en kpi-service
- **Reviewer en GitHub:** Nachovn12

## Contexto de Negocio — Grupo Cordillera

El `kpi-service` contiene la lógica más crítica del sistema de Grupo Cordillera: el patrón Factory Method con `KpiFactory` que selecciona el algoritmo de cálculo correcto para cada tipo de indicador. Si esta lógica tiene bugs sin detectar, los KPIs ejecutivos mostrarán valores incorrectos y los directivos tomarán decisiones basadas en datos erróneos.

El quality gate JaCoCo al 60% garantiza que `KpiFactory`, `KpiService` y `KpiController` tienen cobertura verificada antes de cada merge a `main`. Al ser este el servicio con mayor concentración de lógica de negocio (4 calculadoras + reglas de recálculo), la cobertura es especialmente valiosa para demostrar la calidad de la implementación durante la defensa oral del EP3.

La BD de pruebas es H2 en memoria — los tests no requieren `kpi_db` MySQL levantado, permitiendo ejecución en cualquier entorno CI/CD.
...
El quality gate debe usar `INSTRUCTION` como counter (no `LINE`) para medir cobertura de instrucciones bytecode. El build debe fallar con `"Coverage check failed for project: kpi-service"` si la cobertura cae bajo el 60%.
...
git checkout -b feature/cord-119-jacoco-kpi-service
```

### Paso 1 — Modificar kpi-service/pom.xml

Agregar ejecución `check` al plugin jacoco-maven-plugin:

**Archivo:** `kpi-service/pom.xml`

```xml
<!-- Agregar dentro del plugin jacoco-maven-plugin -->
<execution>
    <id>check</id>
    <phase>verify</phase>
    <goals>
...
Verificar dependencia H2 en `pom.xml`:
```xml
<dependency>
...
**Archivo:** `kpi-service/src/test/resources/application-test.properties`
            ...
            <configuration>
                <target>
                    <copy file="${project.build.directory}/site/jacoco/index.html"
                          tofile="${project.basedir}/../docs/jacoco-kpi-service.html"
                          failonerror="false"/>
                </target>
            </configuration>
        </execution>
...
```bash
git add .
git commit -m "feat(cord-119): configurar JaCoCo quality gate 60% en kpi-service"
git push origin feature/cord-119-jacoco-kpi-service
```

**2. Crear Pull Request en GitHub:**

```bash
gh pr create --base main --head feature/cord-119-jacoco-kpi-service \
  --title "[CORD-119] Quality gate JaCoCo 60% kpi-service" \
  --body "## Cambios realizados\nConfiguración quality gate JaCoCo 60% en kpi-service/pom.xml\n\n## Tests\nKpiFactoryTest, KpiServiceTest, KpiControllerTest\n\n## Cobertura JaCoCo\n>=60%" \
  --reviewer Nachovn12
```

**3. En Jira:**
- Cambiar estado de la HU CORD-119 y sub-tasks a "Finalizada"
...
**Objetivo:** Generar docs/jacoco-kpi-service.html tras cada mvn verify

**Verificar:**
```bash
ls docs/jacoco-kpi-service.html
# El archivo debe existir con métricas de KpiFactory, KpiService, KpiController
```

### Sub-task 3 [CORD-164]: Validar mvn verify con cobertura menor al 60%

**Objetivo:** Demostrar que el quality gate es efectivo

**Pasos:**
1. Comentar `KpiFactoryTest.java` y `KpiServiceTest.java`
2. Ejecutar:
```bash
mvn verify -pl kpi-service
# Esperado: BUILD FAILURE — "Coverage check failed for project: kpi-service"
```
3. Restaurar tests
...
6. Guardar como `docs/jacoco-kpi-service.png`
...
- **Dado** la configuración con KpiFactoryTest, KpiServiceTest y KpiControllerTest
- **Cuando** se ejecuta `mvn clean verify -pl kpi-service`
- **Entonces** genera `target/site/jacoco/index.html` con ≥ 60% de cobertura y BUILD SUCCESS

**AC2: Quality gate rechaza cobertura insuficiente**
- **Dado** que se comentan KpiFactoryTest y KpiServiceTest
- **Cuando** se ejecuta `mvn verify -pl kpi-service`
- **Entonces** BUILD FAILURE con "Coverage check failed for project: kpi-service"

## DoD (Definition of Done)

- [ ] Rama `feature/cord-119-jacoco-kpi-service` creada y pusheada
...
- [ ] Screenshot guardado en `docs/jacoco-kpi-service.png`
- [ ] Ticket CORD-119 en Jira en estado "Finalizado" con comentario técnico y screenshot
